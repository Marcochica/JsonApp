...
class UpdatejsonController extends Controller
{
    ...
    public function updateJson(Request $request){
        if($request->urlfolder){
            $UrlSinC = substr($request->urlfolder,3);
            $allFiles = Storage::disk('c_drive')->allFiles($UrlSinC);
            if(sizeof($allFiles)){
                foreach ($allFiles as $fil) {
                    $ht=file_get_contents('C:/'.$fil);
                    ...
                    foreach ($users as $user => $value) {
                        ...
                        if($consults !== null){
                            ...
                            foreach ($consults as $consult) {
                                // Cambios en campos
                                $info['usuarios'][$contador]['servicios']['consultas'][$consu]['causaMotivoAtencion'] = "10";
                        ...
                        File::put('C:/'.$fil, $updatedJsonUser);
                        $contador++;
                    }
                }
                ...
                $message = 'La carpeta ingresada no se encuentra en la Unidad C, por favor verifique el nombre de la carpeta y vuelva a intentarlo.';
                return view('updatejson/message', compact('message'));
            }
        }
    }
